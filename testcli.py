...
def main():
    ...
    parser.add_argument('-E', '--expression', dest='expressions_list', action='append',
                        nargs=3)
    ...
    parser.add_argument('repl', metavar='repl', type=str, nargs='?',
                        # default='world',
                        help='and a repl')
    ...
    if exes := vargs.pop('expressions_list', []):
        ...
        if args.pattern and not args.file:
            ...
            # this is bad!
            parser.error(f"""Got an unexpected positional argument; either:
