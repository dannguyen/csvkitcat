...
# test -r accepts multiple column names
    ...
    def test_count_multi_rows(self):
        self.assertRows(['-r', 'gender,race', "examples/peeps.csv"], [
            ['gender', 'race', 'Count'],
            ['female', 'white', '1'],
            ['female', 'black', '2'],
            ['female', 'asian', '1'],
    ...
    def test_count_multi_rows_and_col(self):
        self.assertLines(['-r', 'gender,race', "-c", "name", "examples/peeps.csv"], [
...
# Count, Max, MaxLength, Min, Mean, Median, Mode, StDev, Sum
    ...
    def test_agg_sum(self):
            self.assertRows(['-r', 'gender', '-a', 'sum,age', "examples/peeps.csv"], [
                ['gender',  'Sum'],
                ['female',  '90'],
                ['male',    '45'],
...
# Furie,asian,female,60
    ...
    def test_agg_max(self):
            self.assertRows(['-r', 'race', '-a', 'max,age', "examples/peeps2.csv"], [
                ['race',     'Max'],
                ['white',    '40'],
                ['asian',    '60'],
                ['black',    '40'],
    ...
    def test_agg_mean(self):
            self.assertRows(['-r', 'gender', '-a', 'mean,age', "examples/peeps2.csv"], [
                ['gender',     'Mean'],
                ['female',      '42.5'],
                ['male',        '40'],
            ])
    ...
    def test_agg_median(self):
            self.assertRows(['-r', 'gender', '-a', 'median,age', "examples/peeps2.csv"], [
                ['gender',     'Median'],
                ['female',     '40'],
                ['male',       '40'],
            ])
    ...
    def test_agg_mode(self):
            self.assertRows(['-r', 'gender', '-a', 'mode,age', "examples/peeps2.csv"], [
                ['gender',     'Mode'],
                ['female',      '40'],
                ['male',        '30'],
            ])
    ...
    def test_agg_min(self):
            self.assertRows(['-r', 'race', '-a', 'min,age', "examples/peeps2.csv"], [
                ['race',     'Min'],
                ['white',    '40'],
                ['asian',    '30'],
                ['black',    '30'],
    ...
    def test_agg_max_length(self):
            self.assertRows(['-r', 'race', '-a', 'maxlength,name', "examples/peeps2.csv"], [
                ['race',     'MaxLength'],
                ['white',    '5'],
                ['asian',    '5'],
                ['black',    '11'],
    ...
    def test_agg_stdev(self):
            self.assertRows(['-r', 'gender', '-a', 'stdev,age', "examples/peeps2.csv"], [
                ['gender',     'StDev'],
    ...
    # Alice,
    # Alice,7
    @skiptest("cannot escape ValueError: https://github.com/wireservice/agate/issues/714")
    def test_agg_max_w_null_col(self):
            self.assertRows(['-r', 'name', '-a', 'max,items', "examples/p-items.csv"], [
                ['name', 'Max'],
                ['Alice', '7'],
                ['Bob',  '2'],
                ['Chaz', ''],
...
# test -a count,col_name,col_val
    ...
    def test_agg_count_non_nulls(self):
            self.assertRows(['-r', 'name', '-a', 'count,items', "examples/p-items.csv"], [
                ['name',   'Count'],
                ['Alice',  '2'],
                ['Bob',  '2'],
                ['Chaz',  '0'],
    ...
    def test_agg_count_value_matches(self):
            self.assertRows(['-r', 'name', '-a', 'count,greet,"hey, you"', "examples/greets.csv"], [
                ['name',   'Count'],
                ['Alice',   '1'],
                ['Bob',     '2'],
                ['Cal',     '0'],
...
### Tests that verify my examples
    ...
    def test_examples_count_multi_rows(self):
        self.assertLines(['-r', 'race,gender', "examples/peeps.csv"],
...
## error testing
    ...
    def test_error_when_no_row_or_column(self):
        ...
        assert "At least either --row-pivot or --column-pivot must be specified. Both cannot be empty" in str(e.exception)
    ...
    def test_error_when_invalid_aggregation_specified(self):
        ...
        assert 'Invalid aggregation: "just magic!". Call -a/--agg without a value to get a list of available aggregations' in str(e.exception)


    def test_error_when_aggregating_on_null_column(self):
        with self.assertRaises(ValueError) as e:
            u = self.get_output(['-r', 'name', '-a', 'max,items', "examples/p-items.csv"])
    ...
    def test_error_when_aggregating_non_numer_column(self):
        with self.assertRaises(DataTypeError) as e:
            u = self.get_output(['-r', 'gender', '--agg', 'sum,name', "examples/peeps.csv"])

        assert 'Sum can only be applied to columns containing Number data' in str(e.exception)
